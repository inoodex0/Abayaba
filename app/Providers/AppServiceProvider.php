...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
       $shurjopay = PaymentGateway::where(['status' => 1, 'type' => 'shurjopay'])->first();
        ...
        $pages = CreatePage::where('status',1)->where('pageFor','ful')->limit(10)->get();
        view()->share('pages',$pages);

        $pagesright = CreatePage::where('status',1)->where('pageFor','fl')->limit(10)->get();
        view()->share('pagesright',$pagesright);

        $mainmenu = CreatePage::where('status',1)->where('pageFor','hmm')->get();
