...
class FrontendController extends Controller
{
   public function appconfig()
    ...
    public function hotdealproduct(){
        $data = Product::where('is_deleted',0)->where(['status'=>1])->select('id','slug','name','topsale','old_price','new_price')->with('image')->get();
        return response()->json(['status' => 'success','message'=>'Data fatch successfully','data'=>$data]);
   }
...
//   Home Page Function End ====================
    ...
    public function catproduct($id){
        ...
        $data = Product::where('is_deleted',0)->where(['status'=>1, 'category_id'=>$category->id])->select('id','slug','name','old_price','new_price', 'category_id')->with('image')->orderBy('id','DESC')->get();
