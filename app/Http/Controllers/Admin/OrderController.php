...
class OrderController extends Controller
{
    public function index($slug,Request $request){
        if($slug == 'all'){
            $order_status = (object) [
                'name' => 'All',
                'orders_count'=> Order::where('order_status','!=',8)->count(),
            ];
            $show_data = Order::latest()->with('shipping','status')->where('order_status','!=',8);
            if($request->keyword){
    ...
    public function stock_report(Request $request){
        $products = Product::where('is_deleted',0)->select('id', 'name','new_price','stock')
            ->where('status', 1);
        if ($request->keyword) {
    ...
    public function order_create(){
        $products = Product::where('is_deleted',0)->select('id','name','new_price','product_code')->where(['status'=>1])->get();
    ...
    public function order_store(Request $request){
        ...
        $subtotal = Cart::instance('pos_shopping')->content()->filter(fn($item) => $item->options->is_deleted == 0)->sum(fn($item) => $item->qty * $item->price);
    ...
    //     $product = Product::where('is_deleted',0)->select('id','name','stock','new_price','old_price','purchase_price','slug')->where(['id' => $request->id])->first();
    //     $qty = 1;
    ...
    public function cart_add(Request $request){
        $product = Product::where('is_deleted', 0)
            ->select('id', 'name', 'stock', 'new_price', 'old_price', 'purchase_price', 'slug')
            ->where('id', $request->id)
            ->first();
        ...
        $cartItem = $cartInstance->search(function ($cartItem, $rowId) use ($product) {
            return $cartItem->id === $product->id;
        })->first();

        if ($cartItem) {
            $cartinfo = $cartInstance->update($cartItem->rowId, [
                'qty' => $cartItem->qty + 1,
                'options' => $cartItem->options->merge(['is_deleted' => 0]),
            ]);
        } else {
            $cartinfo = $cartInstance->add([
                'id' => $product->id,
                'name' => $product->name,
                'qty' => 1,
                'price' => $product->new_price,
    ...
    public function cart_remove(Request $request){
        ...
        $cartInstance = Cart::instance('pos_shopping');
        $cartItem = $cartInstance->content()->where('rowId', $request->id)->first();
        $cartInstance->update($request->id, [
            'options' => $cartItem->options->merge(['is_deleted' => 1]),
        ]);
    ...
    public function product_discount(Request $request){
        // return  $cartinfo = Cart::instance('pos_shopping')->content();
        ...
        $cartinfo = Cart::instance('pos_shopping')->update($request->id, [
            'options' => [
                ...
                'is_deleted' => $cart->options->is_deleted,
            ],
        ]); 
        ...
        foreach($orderdetails as $ordetails){
            $cartinfo = Cart::instance('pos_shopping')->add([
                ...
                'options' => [
                    ...
                    'is_deleted' => $ordetails->is_deleted,
                ],
    ...
    public function order_update(Request $request){
        ...
        // order details data removed

        // $detailsIds = Cart::instance('pos_shopping')->content()->map(function ($cart) {
        //     return $cart->options->details_id;
        // });
        // OrderDetails::where('order_id', $order->id)->whereNotIn('id', $detailsIds)->delete();
       ...
       // order details data save
        foreach(Cart::instance('pos_shopping')->content() as $cart){
            ...
            if($exits){
                ...
                $order_details->sale_price       =   $cart->price;
                $order_details->qty              =   $cart->qty;
                // $order_details->is_deleted       =   '0';
                $order_details->is_deleted = ($cart->options->is_deleted) ? '1' : '0';
                $order_details->save();
            }else{
                $order_details                   =   new OrderDetails();
                $order_details->order_id         =   $order->id;
