...
<tr style="{{$value->options->is_deleted == '1' ? 'background:#dbd7d7;' : ''}}">
  <td><img height="30" src="{{asset($value->options->image)}}"></td>
  <td>{{$value->name}}</td>
  <td>
    <div class="qty-cart vcart-qty">
      @if($value->options->is_deleted == '0')
      <div class="quantity">
      ...
      @else
          <p style="text-align:center; font-size:18px;">{{$value->qty}}</span>
      @endif
  </div>
  </td>
  <td>{{$value->price}}</td>
  <td class="discount">
    ...
    @else
          <p style="text-align:center; font-size:18px;">{{$value->options->product_discount}}</span>
    @endif
  </td>
  ...
  <td>
    @if($value->options->is_deleted == '0')
    <button type="button" onclick="removeItem(this)" class="btn btn-danger btn-xs cart_removell" data-id="{{$value->rowId}}"><i class="fa fa-times"></i></button>
    @endif
  </td>
</tr>
