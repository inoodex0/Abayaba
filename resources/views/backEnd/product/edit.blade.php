          ...
          <form action="{{route('products.update')}}" method="POST" class="row" data-parsley-validate="" enctype="multipart/form-data" name="editForm">
            ...
            <div class="col-sm-6">
              <div class="form-group mb-3">
                ...
                <small id="nameCount" class="text-muted">
                  {{ strlen($edit_data->name) }} characters
                </small>
                @error('name')
            ...
            <div class="col-sm-6">
              <div class="form-group mb-3">
                ...
                <input type="text" class="form-control @error('sku') is-invalid @enderror" name="sku" value="{{$edit_data->sku }}" id="sku" required="" />
                @error('sku')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="col-sm-6">
              <div class="form-group mb-3">
                ...
                <select class="form-control form-select select2 @error('category_id') is-invalid @enderror" name="category_id" value="{{ old('category_id') }}" required id="category_id" >
                  <optgroup>
            ...
            <div class="col-sm-4 mb-3">
              ...
              <div class="product_img">
                ...
                <img src="{{asset($image->image)}}" class="edit-image border" alt="{{ $edit_data->img_alt_text }}" />
            ...
            <div class="col-sm-4 mb-3">
              <div class="form-group mb-3">
                ...
                <input type="text" class="form-control @error('img_alt_text') is-invalid @enderror" name="img_alt_text" value="{{$edit_data->img_alt_text }}" id="img_alt_text" required="" />
                @error('img_alt_text')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="col-sm-6">
              <div class="form-group mb-3">
                <label for="slug" class="form-label">Product URL*</label>
                <input type="text" class="form-control @error('slug') is-invalid @enderror" name="slug" value="{{ $edit_data->slug }}" id="slug" />
                @error('slug')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="col-sm-12 mb-3">
              <div class="form-group">
                ...
                <textarea name="specification" rows="6" class="summernote form-control @error('specification') is-invalid @enderror">{{$edit_data->specification}}</textarea>
                @error('specification')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="col-sm-12 mb-3">
              <div class="form-group">
                ...
                <small id="descriptionCount" class="text-muted">
                  {{ strlen(strip_tags($edit_data->description)) }} characters
                </small>
                @error('description')
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const altTextInput = document.getElementById('img_alt_text');

    let isAltTextEdited = false;

    // Detect manual edits to the alt text input
    altTextInput.addEventListener('input', function () {
      isAltTextEdited = true;
    });

    // Update alt text as long as user hasn’t manually edited it
    nameInput.addEventListener('input', function () {
      if (!isAltTextEdited) {
        altTextInput.value = nameInput.value;
      }
    });
  });
</script>
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (nameInput) {
      nameInput.addEventListener('input', function () {
        nameCount.textContent = `${this.value.length} characters`;
      });
    }

    // If using plain textarea (without Summernote)
    if (descriptionTextarea && !$('.summernote').length) {
    ...
    if ($('.summernote').length) {
      $('.summernote').on('summernote.change', function (we, contents) {
        const plainText = $('<div>').html(contents).text(); // Strip HTML
        $('#descriptionCount').text(`${plainText.length} characters`);
      });
    }
  });
</script>
