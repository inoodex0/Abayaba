                ...
                <form action="{{route('pages.update')}}" method="POST" class=row data-parsley-validate=""  enctype="multipart/form-data">
                    ...
                    <div class="col-sm-4">
                        <div class="form-group mb-3">
                            <label for="pageFor" class="form-label">Page For *</label>
                            <select class="form-control @error('pageFor') is-invalid @enderror" name="pageFor" id="pageFor" required>
                                <option value="">Select Page For</option>
                                <option value="hmm" {{ (old('pageFor') ?? ($edit_data->pageFor ?? '')) == 'hmm' ? 'selected' : '' }}>Header Main Menu</option>
                                <option value="ful" {{ (old('pageFor') ?? ($edit_data->pageFor ?? '')) == 'ful' ? 'selected' : '' }}>Footer Useful Link</option>
                                <option value="fl"  {{ (old('pageFor') ?? ($edit_data->pageFor ?? '')) == 'fl'  ? 'selected' : '' }}>Footer Link</option>
                            </select>
                            @error('pageFor')
